        ...
        for i = 1:length(Breaker_UniqID)
            ...
            if length(sameIDs) > 1
                for j = 1:length(sameIDs)
                    Breaker_UniqID(sameID_index(j)) = {char(strcat(Breaker_UniqID(sameID_index(j)),num2str(sameID_index(j))))};
                end
            end
        end
    end
...
if size(XfmrMatrix,1) > 0
    ...
    % Object Regulator Change to NEW From Node
    XfmrSect_Node(:,2)=SecFromTo(BreakerN+SwitchN+RegulatorN+1:BreakerN+SwitchN+RegulatorN+XfmrN,2);
...
SecFromto_ordered=[SecFromTo(BreakerN+SwitchN+RegulatorN+FuseN+1:BreakerN+SwitchN+RegulatorN+FuseN+XfmrN,:);....
    SecFromTo(BreakerN+SwitchN+RegulatorN+1:BreakerN+SwitchN+RegulatorN+FuseN,:);....
    SecFromTo(BreakerN+SwitchN+1:BreakerN+SwitchN+RegulatorN,:);SecFromTo(BreakerN+1:BreakerN+SwitchN,:);SecFromTo(1:BreakerN,:)];
