...
function making_UG_Line_Configuration(conductors_config,FeederName,glm_dir_name,conf_UG_name,conf_UG_phases)
...
for m=1:length(row_index)
    ...
    Rpos=Conductor_type(conf_UG_phases(row_index(m)),5);
    Xpos=Conductor_type(conf_UG_phases(row_index(m)),6);
    Rzero=Conductor_type(conf_UG_phases(row_index(m)),8);
    Xzero=Conductor_type(conf_UG_phases(row_index(m)),9);
